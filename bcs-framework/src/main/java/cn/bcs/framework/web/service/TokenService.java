...
import cn.bcs.common.utils.ip.AddressUtils;
import cn.bcs.common.utils.ip.IpUtils;
import cn.bcs.common.utils.uuid.IdUtils;
...
public class TokenService {
    ...
    public String createToken(LoginUser loginUser) {
        String token = IdUtils.fastUUID() + "_" + loginUser.getTenantId();
    ...
    public Long getTenantId(HttpServletRequest request) {
        ...
        if (StringUtils.isNotEmpty(token)) {
            try {
                ...
                // 解析对应的权限以及用户信息
                String uuid = (String) claims.get(Constants.LOGIN_USER_KEY);
                Long tenantId = getTenantIdByToken(uuid);
                return tenantId;
            } catch (Exception e) {
            }
        }
