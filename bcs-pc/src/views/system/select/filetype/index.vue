  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <div class="top-right-btn">
        <el-col :span="1.5">
          <el-button type="primary" @click="handleAdd" v-hasPermi="['system:fileType:edit']">添加</el-button>
        </el-col>
      </div>
    </el-row>
    ...
    <el-table v-loading="loading" :data="selectDataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="text" @click="handleUpdate(scope.row)" v-hasPermi="['system:fileType:edit']">编辑</el-button>
          <el-button v-if="scope.row.status === '0'" type="text" @click="setStatus(scope.row, 1)"
            v-hasPermi="['system:fileType:status']">禁用</el-button>
          <el-button v-else type="text" @click="setStatus(scope.row, 0)"
            v-hasPermi="['system:fileType:status']">启用</el-button>
          <el-button type="text" @click="handleDelete(scope.row)" v-hasPermi="['system:fileType:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body center :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="分类名称:" prop="name">
          <el-input v-model="form.name" placeholder="请输入分类名称" :maxlength="120" show-word-limit />
        </el-form-item>
        <el-form-item label="备注:" prop="remark">
          <el-input type="textarea" :rows="2" v-model="form.remark" placeholder="请输入备注内容" :maxlength="120"
            show-word-limit />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    // 取消按钮
    cancel() {
      this.open = this.btnLoading = false;
      this.reset();
    },
    // 表单重置
    reset() {
