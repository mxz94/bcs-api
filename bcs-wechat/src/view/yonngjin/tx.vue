<template>
  <div style=" background-color: #f0f0f0;  min-height: 100vh; ">
    ...
    <div class="bg-white mt-[10px] mx-[10px] px-[10px] py-[20px] text-center">
      <van-row>
        <van-col span="12">
          <div class="text-[#424242] text-[12px]">上周待确认佣金</div>
          <div  class="text-[#000000] text-[18px] font-bold py-[10px]">¥{{ user.waitInBalance }}</div>
        </van-col>
        <van-col span="12">
          <div class="text-[#424242] text-[12px]">可提现金额</div>
          <div  class="text-[#000000] text-[18px] font-bold py-[10px]">¥{{ user.balance }}</div>
        </van-col>
      </van-row>
      <van-divider></van-divider>
      <div  class="text-[#000000] text-[12px]">注：最低提现20元, 提现个人所得税8.6%</div>
      <div  class="text-[#000000] text-[12px]">周日才可提现佣金</div>
      <div class="mt-[10px]">
...
export default {
  ...
  methods: {
    ...
    onConfirm() {
      ...
      if (this.tx.amount > this.user.balance) {
        this.$toast('超过可提现金额');
        return
      }
      let that =this
